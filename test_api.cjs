...
async function run() {
  ...
  if (testRecipe) {
    ...
    console.log("Not in recipesStore, checking pendingStore");
    const pRes = await fetch('http://localhost:3000/api/pending-recipes');
    const pData = await pRes.json();
    const pr = pData.data.find(r => r.author_uid === 'test_uid');
    if (pr) {
      console.log("Pending author_name:", pr.author_name);
      console.log("Pending author.name:", pr.author?.name);
    }
  }
}
run();
